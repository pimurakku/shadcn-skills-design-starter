...
1. **Read the reference like a designer.** Infer and write down:
   ...
   - **Layout archetype + sequence** — full-bleed hero / asymmetric split / bento / editorial stack (`.claude/taste/design-taste.md` → Variance Mandate).
2. **Anchor to a known system** if it's close — browse `.claude/taste/aesthetic-systems.md` / `python3 scripts/design_systems.py search <term>` and adopt that recipe to stabilize decisions.
...
4. **Rebuild layout + components** token-driven via `.claude/frameworks/adapter-protocol.md` + `.claude/components/*`: one shared primitive layer, all 8 states, a11y wired, no emoji (lucide), single theme. Apply taste (`design-taste.md`) so it doesn't regress to generic.
