...
## Steps
1. **Brief Inference (mandatory)** — name the industry, audience, the one mood adjective, and motion depth (`.claude/taste/design-taste.md` → Brief Inference). Pick an anchoring archetype from `.claude/taste/aesthetic-systems.md`.
...
4. **Scales** — Major Third type scale + composite text styles, 4px spacing scale, radius tiers, elevation, and `.claude/tokens/motion.json`-style durations/easings.
5. **Emit** the DTCG `.claude/tokens/*.json` (3-tier) + a single `theme.css` (the one shared source, `[data-theme="dark"]` overrides). Optionally feed the token-build pipeline (`token-build` skill) for other platforms.
...
> Output is a verified token foundation — the measurable part is provable (run `npm run verify`). Brand "feel" still benefits from a human review against `.claude/taste/design-taste.md`.
